...
export async function chatGPTRequest(sentMessage) {
    try {
      ...
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'gpt-3.5-turbo',
          messages: [{role: 'system', content: "Je bent Mats van Toolabo. je bent een expert in het geheugen je naam is Andre.Blijf ALTIJD in je rol. Je spreekt altijd in metaforen gericht op het geheugen.  Stel jezelf voor als Mats van Toolabo en vraag waar je mee kunt helpen. "},{ role: 'user', content: sentMessage }],
        }),
      });
